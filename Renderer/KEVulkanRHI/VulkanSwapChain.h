...
public:
	...
	void initVKSurfaceWithSystemSurface(VkSurfaceKHR os_surface)
...
#endif
	{
		...
		{
			fpGetPhysicalDeviceSurfaceSupportKHR(physicalDevice, i, os_surface, &supportsPresent[i]);
		}
		...
		{
			assert(0&&"Could not find a graphics and/or presenting queue!");
		}

		// todo : Add support for separate graphics and presenting queue
		if (graphicsQueueNodeIndex != presentQueueNodeIndex)
		{
			assert(0 && "Separate graphics and presenting queues are not supported yet!");
		}
		...
		uint32_t formatCount;
		 (fpGetPhysicalDeviceSurfaceFormatsKHR(physicalDevice, os_surface, &formatCount, NULL));
		assert(formatCount > 0);

		std::vector<VkSurfaceFormatKHR> surfaceFormats(formatCount);
		 (fpGetPhysicalDeviceSurfaceFormatsKHR(physicalDevice, os_surface, &formatCount, surfaceFormats.data()));
